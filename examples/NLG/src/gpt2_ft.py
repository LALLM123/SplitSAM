...
def train_validate(
    ...
    for idx, data in enumerate(train_loader):
        ...
        if train_step % args.save_interval == 0:
            if args.rank == 0:
                if args.model_card=='gpt2.md':
                    model_path = os.path.join("/mnt/data/hxj/trained_models/GPT2_M/e2e", f'model.{train_step}_r={args.lora_dim}_seed={args.random_seed}.pt')
                if args.model_card=='gpt2.sm':
                    model_path = os.path.join("/mnt/data/hxj/trained_models/GPT2_S/e2e", f'model.{train_step}_r={args.lora_dim}_seed={args.random_seed}.pt')
                #model_path = os.path.join("/mnt/data/hxj/trained_models/GPT2_S/e2e", f'model.{train_step}.r={args.lora_dim}.pt')
        ...
        if train_step == args.max_step:
            ...
            df.to_excel(f'{args.model_card} rank={args.lora_dim} seed={args.random_seed}_cen.xlsx', sheet_name='Sheet1',index=False)
            break
    '''
    if args.rank == 0:
        model_path = os.path.join(args.work_dir, f'model.{train_step}.r={args.lora_dim}.pt')
        print('saving checkpoint', model_path)
        torch.save({'model_state_dict': model.state_dict()}, model_path)
    '''
    distributed_sync(args)
